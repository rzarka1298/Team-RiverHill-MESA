...
layout = [
            [sg.Text('PH Value too high/low'), sg.Listbox(options, select_mode=sg.LISTBOX_SELECT_MODE_SINGLE, size=(20, len(options)))],
...
class atlas_i2c:
    ...
    def read(self, num_of_bytes=31):
        ...
        if response[0] == 1:  # if the response isnt an error
            ...
            char_list = [chr(x & ~0x80) for x in list(response[1:5])]
